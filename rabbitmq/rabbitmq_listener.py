...
class RabbitMQListener:
    ...
    def __verification_callback(self, ch, method, properties, body):
        ...
        try:
            EmailService.send_verification_email(verification)
            ...
            self._channel.basic_publish(
                ...
                body=json.dumps(verification).encode('utf-8'))
            self.logger.info("Acknowledging verification email.")
            ...
            self._channel.basic_publish(
                ...
                properties=pika.BasicProperties(correlation_id=properties.correlation_id),
                body=json.dumps(verification).encode('utf-8'))
            self.logger.info("Acknowledging failed message.")
            ch.basic_ack(delivery_tag=method.delivery_tag)
    ...
    def __password_reset_callback(self, ch, method, properties, body):
        ...
        try:
            ...
            self._channel.basic_publish(
                ...
                routing_key=properties.reply_to,
                properties=pika.BasicProperties(correlation_id=properties.correlation_id),
                body=json.dumps(pass_reset_request).encode('utf-8'))
            self.logger.info("Acknowledging password reset email.")
            ch.basic_ack(delivery_tag=method.delivery_tag)
        except Exception as ex:
            self._channel.basic_publish(
                exchange='',
    ...
    def __notification_callback(self, ch, method, properties, body):
        ...
        try:
            ...
            self.logger.info("Acknowledging notification email.")
            ch.basic_ack(delivery_tag=method.delivery_tag)
        except Exception as ex:
            self._channel.basic_publish(
                exchange='',
